...
# From Manual Data Verification to an Automated Diff Report
...
**TL;DR** — Manual verification of structured data files before a code freeze was error-prone and squeezed into a 2-hour window. I built a small tool (Python and PowerShell) — developed with Copilot in Visual Studio Code as a coding assistant — that compares the target file against the stakeholder input files (.dcm / .txt / Excel), applies the domain rules, and returns a **deviation report**. The reviewer reads a short report instead of re-checking every value by eye. **Result: 83% reduction in working time and zero follow-up errors across 4 cycles, standardized in an SOP and handed over to colleagues.**
...
- **Manual and error-prone**: values were checked by eye; a slip often surfaced only at the next release, when a data deviation finally appeared.
...
- reads the target file and the input file(s) (.dcm / .txt / Excel)
...
Crucially, the AI sits on the **code** side, not the **data** side: everything ran locally on my own machine — the DCM files, the Excel inputs, and the generated diff reports never left it. The AI assistant helped write and iterate the code; the comparison itself is a deterministic local script. **AI for building, not for handling the data** — a deliberate design rule, not an accident.

## 5. Results
- 83% reduction in working time for this task
- Zero follow-up errors across 4 cycles
